using System.Collections.Generic;
using GameClient.Defs;
using GameClient.Misc;
using GameClient.PacketManagers;
using HarmonyLib;
using RimWorld;
using RimWorld.Planet;
using UnityEngine;
using Verse;

namespace GameClient.Patches.Pages
{
    [HarmonyPatch(typeof(Page_CreateWorldParams), "DoWindowContents")]
    public static class PatchWhenHost
    {
        [HarmonyPrefix]
        public static bool DoPre(
            Rect rect,
            Page_CreateWorldParams __instance,
            string ___seedString,
            float ___planetCoverage,
            OverallRainfall ___rainfall,
            OverallTemperature ___temperature,
            OverallPopulation ___population,
            LandmarkDensity ___landmarkDensity,
            List<FactionDef> ___factions,
            float ___pollution)
        {
            if (!SessionHandler.IsGeneratingFreshWorld)
                return true;

            Vector2 buttonSize = new Vector2(150f, 38f);
            Vector2 buttonLocation = new Vector2(rect.xMax - buttonSize.x, rect.yMax - buttonSize.y);

            if (Widgets.ButtonText(new Rect(buttonLocation.x, buttonLocation.y, buttonSize.x, buttonSize.y), "Generate World"))
            {
                __instance.Close();

                if (!___factions.Contains(RTFactionDefOf.RTNeutral)) ___factions.Add(RTFactionDefOf.RTNeutral);
                if (!___factions.Contains(RTFactionDefOf.RTAlly)) ___factions.Add(RTFactionDefOf.RTAlly);
                if (!___factions.Contains(RTFactionDefOf.RTEnemy)) ___factions.Add(RTFactionDefOf.RTEnemy);
                if (!___factions.Contains(RTFactionDefOf.RTFaction)) ___factions.Add(RTFactionDefOf.RTFaction);

                PM_World.SetValuesFromGame(
                    ___seedString,
                    ___planetCoverage,
                    ___rainfall,
                    ___temperature,
                    ___population,
                    ___landmarkDensity,
                    ___factions,
                    ___pollution);

                PM_World.GenerateNormalWorld();
            }

            return true;
        }
    }

    [HarmonyPatch(typeof(Page_CreateWorldParams), "PostOpen")]
    public static class PatchWhenPlayer
    {
        [HarmonyPrefix]
        public static bool DoPre(Page_CreateWorldParams __instance)
        {
            if (SessionHandler.IsGeneratingFreshWorld)
                return true;

            __instance.Close();
            PM_World.GenerateNormalWorld();
            return false;
        }
    }
}