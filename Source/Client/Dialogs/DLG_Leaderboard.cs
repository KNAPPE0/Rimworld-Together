...
namespace GameClient.Dialogs
{
    public class DLG_Leaderboard : DLG_Base
    {
        ...
        // KMH: Backward-compatible constructor accepting dev's LeaderboardFile
        public DLG_Leaderboard(LeaderboardFile file) : this()
        {
            if (file != null && file.Entries != null && file.Entries.Length > 0)
            {
                // Rich entries already populated by PM_Leaderboard; nothing more to do
            }
            else if (file != null && file.Scores != null && file.Scores.Count > 0)
            {
                // Dev-style simple leaderboard: convert to entries for display
                var list = new System.Collections.Generic.List<LeaderboardEntryFile>();
                foreach (var kv in file.Scores)
                {
                    list.Add(new LeaderboardEntryFile
                    {
                        Username = kv.Key,
                        WealthExact = kv.Value,
                        Wealth = (int)kv.Value
                    });
                }
                // No direct setter - trigger a richer request via InfoStepMode to get per-settlement data
            }
        }
        ...
        private void DrawFooter(Rect rect, LeaderboardEntryFile[] entriesOnScreen)
        {
            ...
            if (_selectedIndex >= 0 && entriesOnScreen != null && _selectedIndex < entriesOnScreen.Length)
            {
                ...
                if (!string.IsNullOrEmpty(e.DiscordUsername))
                    player += $" ({e.DiscordUsername})";
        ...
        private void DrawEntryRow(Rect rect, ColumnLayout c, LeaderboardEntryFile e, int rank)
        {
            ...
            string tileStr = e.Tile < 0 ? "?" : e.Tile.ToString();
            string tip =
                $"Player: {player}\n" +
                $"Community: {community}\n" +
                $"Faction: {faction}\n" +
