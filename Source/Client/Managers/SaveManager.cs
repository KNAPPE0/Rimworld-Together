...
namespace GameClient.Managers
{
    public static class SaveManager
    {
        ...
        private static void ParsePacket(byte[] bytes)
        {
            ...
            if (data._stepMode == SaveStepMode.Receive)
                SaveReceiverManager.ReceiveSaveFromServer(data);
    ...
    public static class SaveSenderManager
    {
        ...
        {
            // ────── Push wealth first ──────
            WealthManager.SendCurrentWealth();
            // ────── Push stats next ──────
            StatisticalManager.SendCurrentStats();
            ...
            {
                Printer.Warning($"[SaveSenderManager] Save file not found, skipping send: {SaveManager.SaveFilePath}", LogImportanceMode.Verbose);
                return;
            }
            catch (Exception ex)
            {
                Printer.Error($"[SaveSenderManager] Error reading save file: {ex.Message}");
                return;
            }
            ...
            if (IsIntentionalDisconnect &&
                (IntentionalDisconnectReason == DCReason.SaveQuitToMenu || IntentionalDisconnectReason == DCReason.SaveQuitToOS))
            {
                data._instructions = (int)SaveMode.Disconnect;
            }
            else
            {
                data._instructions = SaveMode.Autosave;
            }
    ...
    public static class SaveReceiverManager
    {
        ...
        private static void OnSaveReceived(SaveData data)
        {
            ...
            {
                double remoteTime = SaveManager.GetRealPlayTimeInteractingFromSave(SaveManager.ServerSaveFilePath);
                double localTime = SaveManager.GetRealPlayTimeInteractingFromSave(SaveManager.SaveFilePath);
