    ...
    public static class OptionsProfileSessionManager
    {
        ...
        public static void Bootstrap()
        {
            ...
            try
            {
                ...
                // If the game crashed while enforced, restore personal backup.
                if (State.IsEnforcedActive && Directory.Exists(BackupPath))
                {
        ...
        {
            // Always request; if server has no profile, it responds "none" silently.
        ...
        public static void ReceiveOptionsProfilePacket(ModConfigData data)
        {
            ...
            if (buffer.ReceivedCount >= buffer.ChunkCount)
            {
                ...
                ApplyEnforcedProfile(profileFolder, buffer.Hash, buffer.UpdatedTicks, extractedCount);
            }
        }
        ...
        private static void WatcherLoop(CancellationToken token)
        {
            while (!token.IsCancellationRequested)
            {
                ...
                if (SessionHandler.CurrentNetworkState == ClientNetworkState.Disconnected) continue;
        ...
        private static void RestoreOnGameExit()
        {
            try
            {
                ...
                RestoreBackupToConfig(softReload: false);
                ClearSessionState();
                SafeDeleteDirectory(BackupPath);
            }
            catch { }
